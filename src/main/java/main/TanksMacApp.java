...
public class TanksMacApp
{
    public static void main(String[] args)
    {
        ...
        if (!Game.isOnlineServer)
        {
            ...
            if (Game.framework == Game.Framework.lwjgl)
            {
                Game.game.window = new LWJGLWindow("Tanks", 1400, 900 + Drawing.drawing.statsHeight, Game.absoluteDepthBase, new GameUpdater(), new GameDrawer(), new GameWindowHandler(), Game.options.graphics.vsync, !Panel.showMouseTarget);
                ((LWJGLWindow)Game.game.window).antialiasingEnabled = Game.options.graphics.antialiasing;
            }
